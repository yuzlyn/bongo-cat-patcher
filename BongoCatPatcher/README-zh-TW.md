...
- 進入多人房間時，將其他玩家的貓依右、上、左、下順序緊貼螢幕邊緣，並旋轉貓咪使底部朝向所貼的邊緣，避免散落在桌面中央。
...
補丁會移除舊版由寶箱彈窗觸發的自動購買。改用 Steam 掉落回呼喚醒商店檢查：只有 `Chest_Token` 或 `Emote_Chest_Token` 已存在、遊戲已將寶箱標記為可領取且支付條件成立時，才呼叫原有的購買與開箱流程。倒數到 `00:00` 而 token 尚未同步時會保持待領取，僅檢查本機庫存，不會重複向 Steam 發送交換請求；token 一出現就立即進入原生交換流程。
